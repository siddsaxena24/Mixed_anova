...
file_path <- "C:/Users/siddh/R/Projects/hourly_data/P70_males_merged_Clean.Rda"
...
# USER Input: Step 1
# Please Replace - ** PLEASE RUN EACH LINE
DV <- "FoodIn.cum.kcal"
Group <- "Group"
Time <- "Time"
Subject <- "Animal"
...
## PLEASE run the below function to ensure proper integration of inputted variables in the program
data_new <- data_new %>%
  rename(
    DV = !!sym(DV),
    Group = !!sym(Group),
...
anova_general <- function(data_new) {
  ...
  treat_maineffect2 <- function(data_new) {
    print("Now running post-hoc tests for significance by time point. Depending on data size, this may take slightly longer than expected")
    ## A common problem occurring in ANOVA datasets is zero variance. The first part of this large function tests for time points with zero variance, and provides this information so the user knows why this step is being skipped
    one_way_test <- data_new %>%
      group_by(Time) %>%
      group_modify(~ {
        # Check for zero variance in DV within each group
        group_variances <- .x %>%
          group_by(Group) %>%
          summarize(variance = var(DV, na.rm = TRUE)) %>%
          pull(variance)
        
        if (any(group_variances == 0)) {
          warning(paste("Zero variance detected at Time:", unique(.x$Time), "- Skipping ANOVA for this group"))
          return(tibble())  # Return an empty tibble if there's a zero variance
        }
        ## While we are already checking for zero variance above, the ANOVA test is still being run with precautionary "error catching"
        ## measures that allow the script to run without malfunctioning due to test incompatibility in the data. If it occurs, reasons for any error in the ANOVA table function below will not be specified by the script
        result <- tryCatch({
          anova_test(data = .x, dv = DV, wid = Subject, between = Group)
        }, error = function(e) {
          warning(paste("ANOVA failed at Time:", unique(.x$Time), "-", e$message))
          return(tibble())  # Return empty tibble on failure
        })
        
        as_tibble(get_anova_table(result))
      }) %>%
      adjust_pvalue(method = "bonferroni") %>%
      as_tibble()
    
    # Check if one_way_test is empty before proceeding
    if (nrow(one_way_test) == 0) {
      message("No valid ANOVA tests were run. Please check your data.")
      return(NULL)
    }
    ...
    # Adjust column names and check for significance
    ...
    if (nrow(detected_sig) > 0) {
      ...
      message("Please see complete table above to record time points where significance was seen")
    } else {
      message("No Significance was detected on the dependent variable between groups at any time point")
    }
  }
  treat_maineffect2(data_new)
  ...
  make_pairwise <- function(data_new) {
    # Calculate the maximum DV values for each Time point
    Max_mass <- data_new %>%
      group_by(Time) %>%
      summarize(max_DV = max(DV, na.rm = TRUE), .groups = 'drop')
    ...
    # Ensure the pairwise results and Max_mass have the same number of rows
    pw <- pw %>%
      left_join(Max_mass, by = "Time") %>%
      mutate(yplacement = max_DV) %>%
      select(-max_DV) # Remove intermediate column if not needed
    
    # View and return the result
    View(pw)
    assign('pw', pw, envir=.GlobalEnv)
    return(pw)
  }
...
## AT this point, please create and execute either final_box1 (unaltered graph) function, or final_box2 (smoothed graph with full statistical notations) function. 
  ...
  cat("\nPlease select which boxplot visualization you would like:\n")
  cat("1. Unaltered Boxplot (final_box1)\n")
  cat("2. Smoothed Boxplot (final_box2)\n")
  boxplot_choice <- as.integer(trimws(readline("Enter your choice (1 or 2): ")))
  ...
  if (boxplot_choice == 1) {
    ...
    message("Invalid choice. Defaulting to final_box1.")
    final_box1(pw)
  }
...
## DO NOT USE BELOW FUNCTION, THIS IS A BACKUP IN CASE OF MALFUNCTION WITH "treatment_maineffect2" function
